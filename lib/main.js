(function() {
  var $, $async, $map, CND, D, HOLLERITH, alert, badge, copy, debug, echo, f, help, hide, info, is_hidden, is_stamped, log, njs_fs, njs_path, rpr, select, stamp, step, suspend, unstamp, urge, warn, whisper,
  ...
  this.$fontlist = (function(_this) {
    return function(S) {
      ...
      template = "{\\($texname){\\cjk\\($texname){}永東國酬愛鬱靈鷹袋南去經三國，東來過五湖东国爱郁灵鹰经来过} AaBbCcDdEeFfghijklmnopqrstuvwxyz}";
  ...
  this.$vertical_bar = (function(_this) {
    return function(S) {
          ...
          for (i = 0, len = chunks.length; i < len; i++) {
            ...
            switch (chunk) {
              ...
              case '】':
                ...
                results.push(send(['tex', "\\mktsVerticalBar{}"]));
                break;
              case '「':
              case '」':
                send(hide(stamp(['#', 'vertical-bar', chunk, copy(meta)])));
                results.push(send(['tex', "{\\mktsFontfileSunexta\\color{red}\\makebox[0mm]{" + chunk + "}}"]));
                break;
              default:
