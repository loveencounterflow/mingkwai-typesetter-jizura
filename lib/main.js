...
(function() {
  ...
  this.$main = (function(_this) {
    return function(S) {
      return D.TEE.from_pipeline([_this.$vertical_bar_divider(S), _this.$boxes_and_circles(S), _this.$most_frequent.with_fncrs.$rewrite_events(S), _this.$dump_db(S), _this.$most_frequent.$read(S), _this.$most_frequent.$assemble(S), _this.$most_frequent.$details_from_glyphs(S), _this.$most_frequent.with_fncrs.$format(S), _this.$most_frequent.with_fncrs.$assemble(S), _this.$dump_db.$format(S), _this.$dictionary_readings_and_gloss(S)]);
    };
  })(this);
  ...
  this.$fontlist = (function(_this) {
    return function(S) {
      ...
      template = "{\\($texname) {\\cjk\\($texname){}𠕋𠕋𧱆}}";
      template = "{\\($texname){}0123456789abcdef 12.514.310 u-8e70 uxb-201a3}\\\\\n{\\($texname){}1111111111111111 12.514.310 u-8e70 uxb-201a3}";
      template = "{\\($texname) {\\cjk\\($texname){}魚□[[⃞⃝⃟⃤⃢  ⃝a ⃝a⃞a⃟a⃠a⃢a⃣a⃤a⃥a}}";
      template = "{\\($texname) {\\cjk\\($texname){}タイショウタイクラス眀快排字噐器学の独立 がくのどくりつ}}";
  ...
  this.$dictionary_readings_and_gloss = (function(_this) {
    return function(S) {
  ...
  this.$boxes_and_circles = (function(_this) {
    return function(S) {
      var matcher, use_boxes_and_circles;
      use_boxes_and_circles = false;
      matcher = /(\(\(|\)\)|\[\[|\]\])/g;
      return $(function(event, send) {
        var chunk, chunks, i, len, meta, name, results, text, type;
        if (select(event, ['!', '('], 'JZR.boxes-and-circles')) {
          send(stamp(event));
          return use_boxes_and_circles = true;
        } else if (select(event, ')', 'JZR.boxes-and-circles')) {
          send(stamp(event));
          return use_boxes_and_circles = false;
        } else if (use_boxes_and_circles && select(event, '.', 'text')) {
          type = event[0], name = event[1], text = event[2], meta = event[3];
          chunks = text.split(matcher);
          results = [];
          for (i = 0, len = chunks.length; i < len; i++) {
            ...
            switch (chunk) {
              case '((':
                results.push(send(['tex', "\\mktsCircledLetter{"]));
                break;
              case '[[':
                results.push(send(['tex', "\\mktsBoxedLetter{"]));
                break;
              case '))':
              case ']]':
